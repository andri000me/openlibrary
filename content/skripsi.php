...
elseif(isset($_GET['update-skripsi'])){
    //update_skripsi();
    if($_GET['update-skripsi']=='info'){
        update_skripsi_info();
    }
    elseif($_GET['update-skripsi']=='abstrak'){
        form_update_skripsi_abstrak();
    }
    elseif($_GET['update-skripsi']=='pengarang'){
        form_update_skripsi_pengarang();
    }
    elseif($_GET['update-skripsi']=='penerbit'){
        form_update_skripsi_penerbit();
    }
    elseif($_GET['update-skripsi']=='sirkulasi'){
        form_update_skripsi_sirkulasi();
    }
}
else{
    skripsi_list();
...
<?php
function skripsi_list(){
?>
<div class="row pad-botm">
    <div class="col-md-12">
        <table width="100%">
            <tr>
                <td width="90%"><h4 class="header-line">Koleksi Skripsi Mahasiswa</h4></td>
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `skripsi`");
    ...
    <table width="100%">
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".($noPage-1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".$x."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".($noPage+1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
                <td width="20%"></td>
                <td width="20%" align="">
                    <form role="form" method="post" action="?page=skripsi">             
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM skripsi ORDER BY judul ASC LIMIT $start,$per_page");
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `skripsi`");
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `skripsi` WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%')");
      $page = (isset($_GET['pages'])) ? (int)$_GET['pages'] : 1;
      $start = ($page-1) * $per_page; 
      $pages = ceil(mysql_result($page_query, 0) / $per_page);
      ?>
        ...
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".($noPage-1)."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                if (($showPage != ($pages - 1)) && ($x == $pages))
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".$x."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=skripsi&pages=".($noPage+1)."&search=".$search,"' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM skripsi WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%') ORDER BY judul ASC LIMIT $start,$per_page");
        $jml    = mysql_num_rows($sql);
        if($jml >=1){
            while($data=mysql_fetch_array($sql)){
                $kd = $data['kd_skripsi'];
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `skripsi` WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%')");
$total = mysql_num_rows($jml);
?>
    <strong>Jumlah Record : <?php echo $total; ?></strong>
</div>
...
function detail_skripsi(){
    $id     = $_GET['detail-id'];
    $no     = 1;
    $sql    = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$id'");
    ...
    <div class="col-md-12">           
        <!--    Context Classes  -->
            <div class="panel panel-default">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active in" id="home">
                                    ...
                                    <tbody>                             
                                        <tr>
                                            <td align="center" rowspan="6" width="15%">
                                                <div class="item active">                                            
                                                    <img src="assets/img/skripsi.png" alt="" />                          
                                                </div>
                                                <?php //echo $no++; ?></td>                                                
                                            <td width="10%">Kode</td>
                                            <td>
                                                <?php echo $data['kd_skripsi']; ?></td>
                                        </tr>
                                        <tr>
                                            <td>ISSN ISBN</td>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                        <a href="?page=skripsi&update-skripsi=info&id=<?php echo $data['kd_skripsi'];?>" name="edit">
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=skripsi&update-skripsi=abstrak&id=<?php echo $data['kd_skripsi'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>                                    
                                </tbody>
                            </table>       
                            ...
                            <table class="table">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=skripsi&update-skripsi=pengarang&id=<?php echo $data['kd_skripsi'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>              
                            ...
                            <table class="table ">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=skripsi&update-skripsi=penerbit&id=<?php echo $data['kd_skripsi'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>                 
                            ...
                            <?php                            
                                            ...
                                            $result = mysql_query("SELECT * FROM download_ebook WHERE kd_ebook='$data[kd_skripsi]'");
                                ...
                                ?>                                            
                                    ...
                                    <ul class="manager">
                                        <li><a href="?page=download&id=<?php echo $data['kd_skripsi'];?>&file=<?php echo $data['file'] ?>" target="_blank" >
                                            <?php echo $data['judul']; ?>
                ...
                <a href="?page=skripsi"> <i class="fa fa-reorder"></i> Kembali </a>
        </div>
    </div>
</div>
<!----------------------------------------------------------------------------------------------------------------------------------------------------------- -->
...
?>
    <div class="panel panel-info">
        <div class="panel-heading">
            FORM INPUT SKRIPSI BARU
        </div>
        <div class="alert alert-warning">
            Upload file Anda dengan melengkapi form di bawah ini. File yang bisa di Upload hanya file dengan ekstensi <b>.doc, .docx, .xls, .xlsx, .ppt, .pptx, .pdf, .rar, .zip</b> dan besar file (file size) maksimal hanya 5 MB.
        </div>
        <div class="panel-body">
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><div class="form-group">
                    ...
                    <tr>
                        ...
                        <td width="70%"><br/><button type="submit" class="btn btn-info" name="save">Simpan </button>
                                            <a href="?page=skripsi" class="btn btn-danger"> Batal </a></td>
                    </tr>
                </table>
            </form>
        </div>
...
function update_skripsi_info(){
$kd_skripsi  = $_GET['id'];
$query       = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());
...
if(empty($file_name)){   //jika gambar kosong atau tidak di ganti
    $q  = mysql_query("UPDATE skripsi SET
            kd_skripsi  = '$_POST[kd_skripsi]',
            ...
            status      = '$_POST[status]',
            ket         = '$_POST[ket]'
            WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());

                if($q){
            ?>
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=skripsi&detail-id=<?php echo $data['kd_skripsi'];?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
...
if(in_array($file_ext, $allowed_ext) === true){
    if($file_size < 1044070){
        $lokasi = 'assets/bp/'.$kd_skripsi.'.'.$file_ext;
...
?>
                    <div class="alert alert-success alert-dismissable">
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                        Data berhasil di input <a href="?page=skripsi&detail-id=<?php echo $data['kd_skripsi'];?>" class="alert-link">Kembali</a>
                    </div>
<?php 
                }else{
                    echo '<div class="alert alert-danger alert-dismissable">
    ...
    <div class="panel panel-info">
        ...
        <div class="panel-body">
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    <tr>
                        <td width="20%"><div class="form-group"><label>Kode skripsi</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><input class="form-control" type="text" name="kd_skripsi" readonly value="<?php echo $data['kd_skripsi'];?>" /></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>Judul skripsi</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><input class="form-control" type="text" name="judul" value="<?php echo $data['judul'];?>" autofocus required /></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>ISSN ISBN</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><input class="form-control" type="text" name="isbn" value="<?php echo $data['issn_isbn'];?>" /></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>Jurusan</label></div></td>
                        <td width="5%">:</td>
                    ...
                    <tr>
                        ...
                        <td width="70%"><div class="form-group"><button type="submit" class="btn btn-info" name="update">Simpan </button>
                                        <a href="?page=skripsi&detail-id=<?php echo $kd_skripsi;?>" class="btn btn-danger"> Batal </a></div></td>
                    </tr>
                </table>
            </form>
        </div>
...
function form_update_skripsi_abstrak(){
    $kd_skripsi=$_GET['id'];
    $query  = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
    ?>
...
$abstrak       = mysql_real_escape_string($_POST['abstrak']);
    ...
    $q  = mysql_query("UPDATE skripsi SET
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=skripsi&detail-id=<?php echo $kd_skripsi ;?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
}
...
function form_update_skripsi_pengarang(){
    $kd_skripsi=$_GET['id'];
    $query  = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
...
function form_update_skripsi_penerbit(){
    $kd_skripsi=$_GET['id'];
    $query  = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
...
$date       = date("Y-m-d");
    ...
    $q2  = mysql_query("UPDATE skripsi SET
...
/*function update_skripsi(){
    if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {
$kd_skripsi  = $_GET['update-id'];
$query       = mysql_query("SELECT * FROM skripsi WHERE kd_skripsi='$kd_skripsi'") or die (mysql_error());
$data        = mysql_fetch_array($query);
            ...
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    ...
                    <tr>
                        <td width="20%"><div class="form-group"><label>Abstrak</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><textarea class="form-control" rows="3" name="abstrak"><?php echo $data['abstrak'];?></textarea></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>Status</label></div></td>
                        <td width="5%">:</td>
                    ...
                    <tr>
                        <td width="20%"></td>
                        <td width="5%"></td>
                        <td width="70%"><br/><button type="submit" class="btn btn-info" name="update">Simpan </button></td>
                    </tr>
                </table>
            </form>
        </div>
...
function input_skripsi(){
if(empty($_POST['judul'])) {
            echo '<div class="alert alert-danger"> Data Gagal Disimpan. Isikan Judul skripsi! </div>';}
        ...
        elseif(empty($_POST['abstrak'])){ 
            ...
            $no5        = "skp";
            $qqq5       = mysql_query("SELECT max(kd_skripsi) AS last FROM skripsi WHERE kd_skripsi LIKE '$no5%'");
            $dd5        = mysql_fetch_array($qqq5);
            $lastkd5    = $dd5['last'];
            $lastNo5    = substr($lastkd5, 3, 4); 
            $nextNo5    = $lastNo5 + 1; 
            $kd_skripsi  = $no5.sprintf('%04s', $nextNo5);

            $kd_petugas = $_SESSION['user'];

                if(in_array($file_ext, $allowed_ext) === true){
                 if($file_size < 5000000){
                    $lokasi = 'upload_file/'.$kd_skripsi.'.'.$file_ext;
                    move_uploaded_file($file_tmp, $lokasi);

            $q          = mysql_query("INSERT INTO skripsi VALUES(
                        '$kd_skripsi',
                        '$kd_petugas',
                        '$judul',
                        '$kd_pengarang',                        
        ...
        if($q){
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di input <a href="?page=skripsi" class="alert-link">Kembali</a>
                </div>
            <?php 
            }else{
            echo '<div class="alert alert-danger alert-dismissable">
        ...
        echo '<div class="alert alert-danger alert-dismissable">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>ERROR: Besar ukuran file (file size) maksimal 3 Mb!</div>';
        }
        }else{
                    echo '<div class="alert alert-danger alert-dismissable">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>ERROR: Ekstensi file tidak di izinkan!</div>';
