...
elseif(isset($_GET['update-makalah'])){
    //update_makalah();
     if($_GET['update-makalah']=='info'){
        update_makalah_info();
    }
    elseif($_GET['update-makalah']=='abstrak'){
        form_update_makalah_abstrak();
    }
    elseif($_GET['update-makalah']=='pengarang'){
        form_update_makalah_pengarang();
    }
    elseif($_GET['update-makalah']=='penerbit'){
        form_update_makalah_penerbit();
    }
    elseif($_GET['update-makalah']=='sirkulasi'){
        form_update_makalah_sirkulasi();
    }
}
else{
    makalah_list();
...
<?php
function makalah_list(){
?>
<div class="row pad-botm">
    <div class="col-md-12">
        <table width="100%">
            <tr>
                <td width="90%"><h4 class="header-line">Koleksi Makalah Mahasiswa</h4></td>
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `makalah`");
    ...
    <table width="100%">
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".($noPage-1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".$x."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".($noPage+1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
                <td width="20%"></td>
                <td width="20%" align="">
                    <form role="form" method="post" action="?page=student-paper">             
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM makalah ORDER BY judul ASC LIMIT $start,$per_page");
        ...
        ?>
            <tr>                                    
                <td><h4><?php echo "<a href=?page=student-paper&detail-id=".$data['kd_makalah'].">".$data['judul']."</a>"; ?></h4>
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `makalah`");
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `makalah` WHERE judul LIKE '%$search%'");
      $page = (isset($_GET['pages'])) ? (int)$_GET['pages'] : 1;
      $start = ($page-1) * $per_page; 
      $pages = ceil(mysql_result($page_query, 0) / $per_page);
      ?>
        ...
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".($noPage-1)."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                if (($showPage != ($pages - 1)) && ($x == $pages))
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".$x."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=student-paper&pages=".($noPage+1)."&search=".$search,"' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
                ...
                <td width="20%" align="">
                    ...
                    <div class="input-group">
                        ...
                        <span class="form-group input-group-btn">
                            <button class="btn btn-default" type="submit">Cari</button>
                            <?php if(isset($_SESSION['user']) AND (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                            <a href="?page=makalah&add-makalah" class="btn btn-primary" type="button"><i class="fa fa-plus"></i></a>
                            <?php } ?>
                        </span>
                    </div>
                    </form>
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM makalah WHERE judul LIKE '%$search%' ORDER BY judul ASC LIMIT $start,$per_page");
        $jml    = mysql_num_rows($sql);
        if($jml >=1){
            while($data=mysql_fetch_array($sql)){
                $kd = $data['kd_makalah'];
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `makalah` WHERE judul LIKE '%$search%'");
$total = mysql_num_rows($jml);
?>
    <strong>Jumlah Record : <?php echo $total; ?></strong>
</div>
...
function detail_makalah(){
    $id     = $_GET['detail-id'];
    $no     = 1;
    $sql    = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$id'");
    ...
    <div class="col-md-12">           
        <!--    Context Classes  -->
            <div class="panel panel-default">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active in" id="home">
                                    ...
                                    <tbody>                             
                                        <tr>
                                            <td align="center" rowspan="5" width="15%">
                                                <div class="item active">                                            
                                                    <img src="assets/img/makalah.png" alt="" />                          
                                                </div>
                                                <?php //echo $no++; ?></td>                                                
                                            <td width="10%">Kode</td>
                                            <td>
                                                <?php echo $data['kd_makalah']; ?></td>
                                        </tr>                                        
                                        <tr>
                                            <td>Klasifikasi</td>
                                            <td><?php 
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                        <a href="?page=student-paper&update-makalah=info&id=<?php echo $data['kd_makalah'];?>" name="edit">
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=student-paper&update-makalah=abstrak&id=<?php echo $data['kd_makalah'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                        </tr> 
                                </tbody>
                            </table>       
                            ...
                            <table class="table">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=student-paper&update-makalah=pengarang&id=<?php echo $data['kd_makalah'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>              
                            ...
                            <table class="table ">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=student-paper&update-makalah=penerbit&id=<?php echo $data['kd_makalah'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>                 
                            ...
                            <?php                            
                                            ...
                                            $result = mysql_query("SELECT * FROM download_ebook WHERE kd_ebook='$data[kd_makalah]'");
                                ...
                                ?>                                            
                                    ...
                                    <ul class="manager">
                                        <li><a href="?page=download&id=<?php echo $data['kd_makalah'];?>&file=<?php echo $data['file'] ?>" target="_blank" >
                                            <?php echo $data['judul']; ?>
                ...
                <a href="?page=student-paper"> <i class="fa fa-reorder"></i> Kembali </a>
        </div>
    </div>
</div>
<!----------------------------------------------------------------------------------------------------------------------------------------------------------- -->
...
function form_makalah(){
?>
    <div class="panel panel-info">
        <div class="panel-heading">
            FORM INPUT MAKALAH BARU
        </div>
        ...
        <div class="panel-body">
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    ...
                    <tr>
                        <td width="20%"><div class="form-group"><label>Unit</label></div></td>
                        <td width="5%">:</td>
                    ...
                    <tr>
                        ...
                        <td width="70%"><br/><button type="submit" class="btn btn-info" name="save">Simpan </button>
                                            <a href="?page=makalah" class="btn btn-danger"> Batal </a></td>
                    </tr>
                </table>
            </form>
        </div>
...
function update_makalah_info(){
$kd_makalah  = $_GET['id'];
$query       = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$kd_makalah'") or die (mysql_error());
...
if(empty($file_name)){   //jika gambar kosong atau tidak di ganti
    $q  = mysql_query("UPDATE makalah SET
            kd_makalah  = '$_POST[kd_makalah]',
            ...
            status      = '$_POST[status]',
            ket         = '$_POST[ket]'
            WHERE kd_makalah='$kd_makalah'") or die (mysql_error());

                if($q){
            ?>
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=student-paper&detail-id=<?php echo $data['kd_makalah'];?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
...
if(in_array($file_ext, $allowed_ext) === true){
    if($file_size < 1044070){
        $lokasi = 'assets/bp/'.$kd_makalah.'.'.$file_ext;
...
?>
                    <div class="alert alert-success alert-dismissable">
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                        Data berhasil di input <a href="?page=student-paper&detail-id=<?php echo $data['kd_makalah'];?>" class="alert-link">Kembali</a>
                    </div>
<?php 
                }else{
    ...
    <div class="panel panel-info">
        ...
        <div class="panel-body">
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    <tr>
                        <td width="20%"><div class="form-group"><label>Kode Makalah</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group">
                            <input class="form-control" type="text" name="kd_makalah" readonly value="<?php echo $data['kd_makalah'];?>" /></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>Judul Makalah</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group">
                    ...
                    <tr>
                        ...
                        <td width="70%"><br/><div class="form-group"><button type="submit" class="btn btn-info" name="update">Simpan </button>
                                        <a href="?page=student-paper&detail-id=<?php echo $kd_makalah;?>" class="btn btn-danger"> Batal </a></td>
                    </tr>
                </table>
            </form>
        </div>
...
function form_update_makalah_abstrak(){
    $kd_makalah=$_GET['id'];
    $query  = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$kd_makalah'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
    ?>
                ...
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td width="70%"><div class="form-group"><button type="submit" class="btn btn-info" name="update">Simpan </button>
                                        <a href="?page=student-paper&detail-id=<?php echo $kd_makalah;?>" class="btn btn-danger"> Batal </a></div></td>
                    </tr>
                </table>
            </form>
        </div>
...
$abstrak       = mysql_real_escape_string($_POST['abstrak']);
    ...
    $q  = mysql_query("UPDATE makalah SET
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=student-paper&detail-id=<?php echo $kd_makalah ;?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
}
...
function form_update_makalah_pengarang(){
    $kd_makalah=$_GET['id'];
    $query  = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$kd_makalah'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
...
function form_update_makalah_penerbit(){
    $kd_makalah=$_GET['id'];
    $query  = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$kd_makalah'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
            ...
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td width="5%"></td>
                        <td width="70%"><div class="form-group"><button type="submit" class="btn btn-info" name="update">Simpan </button>
                                        <a href="?page=student-paper&detail-id=<?php echo $jurnal;?>" class="btn btn-danger"> Batal </a></div></td>
                    </tr>
                </table>
            </form>
        </div>
...
$date       = date("Y-m-d");
    ...
    $q2  = mysql_query("UPDATE makalah SET
...
function form_update_makalah_sirkulasi(){
    $kd_makalah=$_GET['id'];
    $query  = mysql_query("SELECT * FROM makalah WHERE kd_makalah='$kd_makalah'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
    ?>
...
function input_makalah(){
if(empty($_POST['judul'])) {
            echo '<div class="alert alert-danger"> Data Gagal Disimpan. Isikan Judul Makalah! </div>';}
        ...
        elseif(empty($_POST['abstrak'])){ 
            ...
            $no5        = "sp";
            $qqq5       = mysql_query("SELECT max(kd_makalah) AS last FROM makalah WHERE kd_makalah LIKE '$no5%'");
            $dd5        = mysql_fetch_array($qqq5);
            $lastkd5    = $dd5['last'];
            $lastNo5    = substr($lastkd5, 2, 4); 
            $nextNo5    = $lastNo5 + 1; 
            $kd_makalah  = $no5.sprintf('%04s', $nextNo5);

            $kd_petugas = $_SESSION['user'];

                if(in_array($file_ext, $allowed_ext) === true){
                 if($file_size < 1044070){
                    $lokasi = 'upload_file/'.$kd_makalah.'.'.$file_ext;
                    move_uploaded_file($file_tmp, $lokasi);

            $q          = mysql_query("INSERT INTO makalah VALUES(
                        '$kd_makalah',
                        '$kd_petugas',
                        '$judul',
                        '$kd_pengarang',                        
        ...
        if($q){
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di input <a href="?page=student-paper" class="alert-link">Kembali</a>
                </div>
            <?php 
            }else{
            echo '<div class="alert alert-danger alert-dismissable">
