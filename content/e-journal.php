...
elseif(isset($_GET['update-jurnal'])) {
    //update_jurnal();
    if($_GET['update-jurnal']=='info'){
        update_jurnal_info();
    }
    elseif($_GET['update-jurnal']=='abstrak'){
        form_update_jurnal_abstrak();
    }
    elseif($_GET['update-jurnal']=='pengarang'){
        form_update_jurnal_pengarang();
    }
    elseif($_GET['update-jurnal']=='penerbit'){
        form_update_jurnal_penerbit();
    }
    elseif($_GET['update-jurnal']=='sirkulasi'){
        form_update_jurnal_sirkulasi();
    }
}
else{
    jurnal_list();
        ...
        <table width="100%">
            <tr>
                <td width="90%"><h9 class="header-line">
                    <button data-toggle="dropdown" class="btn btn-default dropdown-toggle"><h4> Koleksi Semua E-Journal <span class="caret"></span></h4>
                    </button>
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `jurnal`");
    ...
    <table width="100%">
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".($noPage-1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".$x."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".($noPage+1)."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
                <td width="20%"></td>
                <td width="20%" align="">
                    <form role="form" method="post" action="?page=e-journal">             
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM jurnal ORDER BY judul ASC LIMIT $start,$per_page");
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `jurnal`");
...
<div class="row pad-botm">
    <div class="col-md-12">
        <table width="100%">
            <tr>
                <td width="90%"><h4 class="header-line">Koleksi jurnal Mahasiswa</h4></td>
                <td width="10%"><div align="left">
                    <div class="btn-group">
...
<?php
      $per_page = 10;
      $page_query = mysql_query("SELECT COUNT(*) FROM `jurnal` WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%')");
      $page = (isset($_GET['pages'])) ? (int)$_GET['pages'] : 1;
      $start = ($page-1) * $per_page; 
      $pages = ceil(mysql_result($page_query, 0) / $per_page);
      ?>
        ...
        <tbody>
            <tr>
                <td width="30%">
                    ...
                    if($pages >= 2 && $page <= $pages){
                        if ($noPage > 1 OR $noPage == $pages) 
                            echo  "<a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".($noPage-1)."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\">&lt;&lt; Previous</a>";
                            for($x=1; $x<=$pages; $x++){
                            $showPage = $x;
                            if ((($x >= $noPage - 1) && ($x <= $noPage + 1)) || ($x == 1) || ($x == $pages)){                
                                if (($showPage != ($pages - 1)) && ($x == $pages))
                                ...
                                if ($x == $noPage)
                                    ...
                                    echo " <a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".$x."&search=".$search."' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> ".$x."</a> "; }
                            }
                        if ($noPage < $pages)
                            echo "<a href='".$_SERVER['PHP_SELF']."?page=e-journal&pages=".($noPage+1)."&search=".$search,"' class=\"btn btn-default\" style=\"color:#3366CC; border-bottom-style:groove\"> Next &gt;&gt;</a>";
                    }
                    ?>        
                    </div>
                </td>
...
<div class="table-responsive">
    <table class="table table-hover table-bordered">
        ...
        <?php
        $no     = $start+1;
        $sql    = mysql_query("SELECT * FROM jurnal WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%') ORDER BY judul ASC LIMIT $start,$per_page");
        $jml    = mysql_num_rows($sql);
        if($jml >=1){
            while($data=mysql_fetch_array($sql)){
                $kd = $data['kd_jurnal'];
...
<div align="left">
<?php  
$jml = mysql_query("SELECT * FROM `jurnal` WHERE (judul LIKE '%$search%' OR thn_terbit LIKE '%$search%')");
$total = mysql_num_rows($jml);
?>
    <strong>Jumlah Record : <?php echo $total; ?></strong>
</div>
...
function detail_jurnal(){
    $id     = $_GET['detail-id'];
    $no     = 1;
    $sql    = mysql_query("SELECT * FROM jurnal WHERE kd_jurnal='$id'");
    ...
    <div class="col-md-12">           
        <!--    Context Classes  -->
            <div class="panel panel-default">
                    ...
                    <div class="tab-content">
                        <div class="tab-pane fade active in" id="home">
                                    ...
                                    <tbody>                             
                                        <tr>
                                            <td align="center" rowspan="6" width="15%">
                                                <div class="item active">                                            
                                                    <img src="assets/img/e-journal.png" alt="" />                          
                                                </div>
                                                <?php //echo $no++; ?></td>                                                
                                            <td width="10%">Kode</td>
                                            <td>
                                                <?php echo $data['kd_jurnal']; ?></td>
                                        </tr>
                                        <tr>
                                            <td>ISSN ISBN</td>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                        <a href="?page=e-journal&update-jurnal=info&id=<?php echo $data['kd_jurnal'];?>" name="edit">
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=e-journal&update-jurnal=abstrak&id=<?php echo $data['kd_jurnal'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>                                    
                                </tbody>
                            </table>       
                            ...
                            <table class="table">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=e-journal&update-jurnal=pengarang&id=<?php echo $data['kd_jurnal'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>              
                            ...
                            <table class="table ">
                                ...
                                ?>
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td><?php if(isset($_SESSION['user']) and (($_SESSION['level'])=='petugas' OR ($_SESSION['level'])=='admin')) {?>
                                                    <a href="?page=e-journal&update-jurnal=penerbit&id=<?php echo $data['kd_jurnal'];?>" name="edit">
                                                    <i class="fa fa-pencil"></i> Edit</a><?php } ?></td>
                                    </tr>
                                </tbody>
                            </table>                 
                            ...
                            <?php                            
                                            ...
                                            $result = mysql_query("SELECT * FROM download_ebook WHERE kd_ebook='$data[kd_jurnal]'");
                                ...
                                ?>                                            
                                    ...
                                    <ul class="manager">
                                        <li><a href="?page=download&id=<?php echo $data['kd_jurnal'];?>&file=<?php echo $data['file'] ?>" target="_blank" >
                                            <?php echo $data['judul']; ?>
                ...
                <a href="?page=e-journal"> <i class="fa fa-reorder"></i> Kembali </a>
        </div>
    </div>
</div>
<!----------------------------------------------------------------------------------------------------------------------------------------------------------- -->
                ...
                <table width="100%">
                    ...
                    <tr>
                        ...
                        <td width="70%"><div class="form-group"><div class="form-group">
                                            <select class="form-control" name="kategori" id="kategori" required>
                                                ...
                                                <option value="md">Mahasiswa&Dosen</option>
                                            </select>
                                        </div></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td width="70%"><br/><button type="submit" class="btn btn-info" name="save">Simpan </button>
                                            <a href="?page=e-journal" class="btn btn-danger"> Batal </a></td>
                    </tr>
                </table>
            </form>
        </div>
...
function update_jurnal_info(){
$kd_jurnal  = $_GET['id'];
$query       = mysql_query("SELECT * FROM jurnal WHERE kd_jurnal='$kd_jurnal'") or die (mysql_error());
...
if(empty($file_name)){   //jika gambar kosong atau tidak di ganti
    $q  = mysql_query("UPDATE jurnal SET
            ...
            status      = '$_POST[status]',
            ket         = '$_POST[ket]'
            WHERE kd_jurnal='$kd_jurnal'") or die (mysql_error());

                if($q){
            ?>
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=e-journal&detail-id=<?php echo $data['kd_jurnal'];?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
...
if(in_array($file_ext, $allowed_ext) === true){
    if($file_size < 1044070){
        $lokasi = 'assets/bp/'.$kd_jurnal.'.'.$file_ext;
...
?>
                    <div class="alert alert-success alert-dismissable">
                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                        Data berhasil di input <a href="?page=e-journal&detail-id=<?php echo $data['kd_jurnal'];?>" class="alert-link">Kembali</a>
                    </div>
<?php 
                }else{
    ...
    <div class="panel panel-info">
        ...
        <div class="panel-body">
            <form role="form" action="" method="post" enctype="multipart/form-data">
                <table width="100%">
                    <tr>
                        <td width="20%"><div class="form-group"><label>Kode jurnal</label></div></td>
                        <td width="5%">:</td>
                        <td width="70%"><div class="form-group"><input class="form-control" type="text" name="kd_jurnal" readonly value="<?php echo $data['kd_jurnal'];?>" /></div></td>
                    </tr>
                    <tr>
                        <td width="20%"><div class="form-group"><label>Judul jurnal</label></div></td>
                        <td width="5%">:</td>
                    ...
                    <tr>
                        ...
                        <a href="?page=e-journal&detail-id=<?php echo $kd_jurnal;?>" class="btn btn-danger"> Batal </a></div></td>
                    </tr>
                </table>
            </form>
        </div>
...
function form_update_jurnal_abstrak(){
    $kd_jurnal=$_GET['id'];
    $query  = mysql_query("SELECT * FROM jurnal WHERE kd_jurnal='$kd_jurnal'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
    ?>
...
$abstrak       = mysql_real_escape_string($_POST['abstrak']);
    ...
    $q  = mysql_query("UPDATE jurnal SET
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di update <a href="?page=e-journal&detail-id=<?php echo $kd_jurnal ;?>" class="alert-link">Kembali</a>
                </div>
            <?php 
            }
}
...
function form_update_jurnal_pengarang(){
    $kd_jurnal=$_GET['id'];
    $query  = mysql_query("SELECT * FROM jurnal WHERE kd_jurnal='$kd_jurnal'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
...
function form_update_jurnal_penerbit(){
    $kd_jurnal=$_GET['id'];
    $query  = mysql_query("SELECT * FROM jurnal WHERE kd_jurnal='$kd_jurnal'") or die (mysql_error());
    $data   = mysql_fetch_array($query);
...
$date       = date("Y-m-d");
    ...
    $q2  = mysql_query("UPDATE jurnal SET
...
function input_jurnal(){
if(empty($_POST['judul'])) {
            echo '<div class="alert alert-danger"> Data Gagal Disimpan. Isikan Judul Jurnal! </div>';}
        ...
        elseif(empty($_POST['abstrak'])){ 
            ...
            $no5        = "ej";
            $qqq5       = mysql_query("SELECT max(kd_jurnal) AS last FROM jurnal WHERE kd_jurnal LIKE '$no5%'");
            $dd5        = mysql_fetch_array($qqq5);
            $lastkd5    = $dd5['last'];
            $lastNo5    = substr($lastkd5, 2, 4); 
            $nextNo5    = $lastNo5 + 1; 
            $kd_jurnal  = $no5.sprintf('%04s', $nextNo5);

            $kd_petugas = $_SESSION['user'];

                if(in_array($file_ext, $allowed_ext) === true){
                 if($file_size < 1044070){
                    $lokasi = 'upload_file/'.$kd_jurnal.'.'.$file_ext;
                    move_uploaded_file($file_tmp, $lokasi);

            $q          = mysql_query("INSERT INTO jurnal VALUES(
                        '$kd_jurnal',
                        '$kd_petugas',
                        '$kategori',
                        '$judul',
        ...
        if($q){
            ...
            <div class="alert alert-success alert-dismissable">
                    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button>
                    Data berhasil di input <a href="?page=e-journal" class="alert-link">Kembali</a>
                </div>
            <?php 
            }else{
            echo '<div class="alert alert-danger alert-dismissable">
